...
abstract class DataModel implements JsonSerializable {
    ...
    protected function hydrateFromDatabase() {
        $sql = sprintf("SELECT * FROM %s WHERE %s = '%s';", static::escape(static::getTableName()), static::escape(static::getTablePk()), $this->id());
    ...
    protected function insert() {
        ...
        // build sql statement
        $sql = sprintf("INSERT INTO %s (%s) VALUES (%s)", static::escape(static::getTableName()), implode(', ', $fieldNames), implode(', ', $fieldMarkers));
    ...
    public function update() {
        ...
        foreach ($array AS $key => $value) {
            if (is_object($value) && $value instanceof RawSQL) {
                $fields[] = sprintf('%s = %s', static::escape($key), (string) $value);
            } else {
                $fields[] = sprintf('%s = ?', static::escape($key));
                $types[] = $this->parseValueType($value);
                $values[] = &$array[$key];
            }
        }
        ...
        // build sql statement
        $sql = sprintf("UPDATE %s SET %s WHERE %s = ?", static::escape(static::getTableName()), implode(', ', $fields), $pk);
